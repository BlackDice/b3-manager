	...
	Tree.prototype = {
		...
		*/
		...
		getPathString: function(from_node, to_node, stacked) {
			...
			// decide if the node is on the east side or west side
			...
			P1A = {x: P1.x, y: P1.y - radius}
			P1B = {x: P1.x + (radius * dirH), y: P1.y}

			P2A = {x: P2.x - (radius * dirH), y: P2.y}
			P2B = {x: P2.x, y: P1.y + (radius * dirV)}
			...
			var p2a = P2A.x + ',' + P2A.y;
			var p2b = P2B.x + ',' + P2B.y;
			var ep  = endPoint.x + ',' + endPoint.y;
			var pm  = (P1.x + P2.x)/2 + ',' + (P1.y + P2.y)/2;
			var pathString, stackPoint;
