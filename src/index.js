...
(function() {
  var $activeTreeDesc, $activeTreeName, $nodeList, $select, $subjectList, $treeForm, $treeInput, $treeList, Chief, activeChief, activeNode, activeNodeId, activeSubject, activeSubjectId, addTree, alertify, b3, cActiveTree, cActiveTreeId, chiefList, closeNode, closeSubject, closeTree, dragNode, env, enviroments, firebase, gridSize, handleTreeChange, hideMemoryPanel, imageExists, loadData, loadNodes, loadSubjects, loadTreeList, memory, nodes, openNode, openSubject, openTree, playing, removeTree, setupFirebase, showMemoryPanel, tickInterval, tickIntervalRef, timer, toggleInput, toggleNode, toggleSubject, toggleTree, treantContainer, treeLoader, _, _i, _len;
  ...
  toggleTree = function(cTree, $li) {
    return function() {
      var activeSubject, loadingTreeId;
    ...
    for (_j = 0, _len1 = order.length; _j < _len1; _j++) {
      ...
      _results.push((function() {
        ...
        for (key in category) {
          ...
          _results1.push($li.on('click', toggleNode(node, $li)));
        }
        return _results1;
      })());
    }
  ...
  toggleNode = function(node, $li) {
    return function() {
      var loadingNodeId;
      loadingNodeId = node.name;
      $nodeList.find('li').removeClass('active');
      ...
      if (activeNodeId !== loadingNodeId) {
        return openNode(loadingNodeId, node, $li);
      }
    };
  };

  openNode = function(id, node, $li) {
    activeNodeId = id;
    activeNode = node;
    $li.addClass('active');
  ...
  toggleSubject = function(subject, $li) {
    return function() {
      ...
      if (activeSubjectId !== loadingSubjectId) {
        return openSubject(loadingSubjectId, subject, $li);
      }
    };
  };
